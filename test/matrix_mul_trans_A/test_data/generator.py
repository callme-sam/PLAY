...
def matrix_mul_trans_A(mat_a, mat_b):
    """Performa matrix multiplication."""
    return np.matmul(mat_a.T, mat_b)
...
def generate_header_file(M, N, P, mat_a, mat_b, expected, filename="data.h"):
    ...
    with open(filepath, "w") as f:
        ...
        f.write(f"float mat_a[] = {format_matrix(mat_a)};\n")
        f.write(f"float mat_b[] = {format_matrix(mat_b)};\n")
...
def main():
    ...
    mat_b = generate_matrix(M, P)
    expected = matrix_mul_trans_A(mat_a, mat_b)

    generate_header_file(M, N, P, mat_a, mat_b, expected)
    print(f"File 'data.h' successfully generated for matrices of size {M}x{N} and {P}x{N}.")
