...
static void initialize_matrices()
{
    ...
    src_b = my_alloc(DIM_M * DIM_P * sizeof(float));
    result = my_alloc(DIM_N * DIM_P * sizeof(float));
...
static int test_matrix_mul_trans_A()
{
    ...
    if (is_master_core()) printf("INFO | Running 'matrix_mul_trans_A' test on Spatz Cluster with %d Core Compelx\n", NUM_CC);
