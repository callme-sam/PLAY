...
PI_L1 float src_b[DIM_M * DIM_P] __attribute__((aligned(4)));
PI_L1 float result[DIM_N * DIM_P] __attribute__((aligned(4)));
...
static void initialize_matrices()
{
    ...
    for (int m = 0; m < DIM_M; m++)
        for (int p = 0; p < DIM_P; p++) {
            src_b[m * DIM_P + p] = mat_b[m * DIM_P + p];
    }
...
static void check_result()
{
    ...
    matrix_print(src_b, DIM_M, DIM_P, "src_b");
    matrix_print(result, DIM_N, DIM_P, "result");
    matrix_print(expected, DIM_N, DIM_P, "expected");
#endif  /* PRINT_DATA */
...
static void run_test()
{
    ...
    START_LOOP_STATS();
    START_STATS();
    matrix_mul_trans_A(src_a, src_b, result, m, n, p);
    STOP_STATS();
    END_LOOP_STATS();
...
static int test_matrix_mul_trans_A()
{
    int ret;

#ifdef  ENABLE_LOGGING
    printf("INFO | Running 'matrix_mul_trans_A' test on PULP Cluster with %d cores\n", NUM_CORES);
#endif  /* ENABLE_LOGGING */
...
#ifdef  ENABLE_LOGGING
    printf("INFO | Running 'matrix_mul_trans_A' test on Fabric Controller\n");
#endif  /* ENABLE_LOGGING */
...
#ifdef  ENABLE_LOGGING
    printf("\n##################################### MATRIX_MUL_TRANS_A TEST ####################################\n\n");
#endif  /* ENABLE_LOGGING */

    ret = test_matrix_mul_trans_A();
